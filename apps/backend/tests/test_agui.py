...
class TestAGUIMount:
    ...
    def test_agui_app_type_with_provider(self) -> None:
        """AG-UI app is an AGUIApp when a provider is configured."""
        env = _test_env({"DATAX_OPENAI_API_KEY": "sk-test"})
        with patch.dict(os.environ, env, clear=True):
            app = create_agui_app(cors_origins=["http://localhost:5173"])
            assert type(app).__name__ == "AGUIApp"

    def test_agui_app_type_without_provider(self) -> None:
...
class TestAGUIHandshake:
    ...
    @pytest.mark.asyncio
    async def test_agui_endpoint_responds(self) -> None:
    ...
    @pytest.mark.asyncio
    async def test_agui_cors_allows_configured_origin(self) -> None:
    ...
    @pytest.mark.asyncio
    async def test_agui_cors_blocks_unconfigured_origin(self) -> None:
...
class TestAGUINoProvider:
    ...
    @pytest.mark.asyncio
    async def test_no_provider_returns_error(self) -> None:
    ...
    @pytest.mark.asyncio
    async def test_no_provider_error_on_any_path(self) -> None:
    ...
    @pytest.mark.asyncio
    async def test_no_provider_cors_still_works(self) -> None:
