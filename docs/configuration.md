...
!!! info "Search order"
    ...
    `backend/` directory. **Later files override earlier ones.**
...
root `.env` and backend-specific overrides in `backend/.env.local`.
...
The `create_db_engine()` function in `backend/app/database.py` automatically
...
1. **Startup** — `create_app()` in `backend/app/main.py` instantiates `Settings()`
