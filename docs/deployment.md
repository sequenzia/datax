...
| **Volumes** | `./backend:/app` (bind mount for hot reload), `backend_venv` (cached `.venv`) |
...
| **Volumes** | `./frontend:/app` (bind mount for HMR), `frontend_node_modules` (cached `node_modules`) |
...
- In development, the bind mount (`./backend:/app`) makes uploads
